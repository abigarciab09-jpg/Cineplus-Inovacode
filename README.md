# Cineplus-Inovacode
CinemaPlus software consists of 8 screens, including a login screen, main menu, movies menu, tickets and showtimes, combo options, individual products, and a purchase receipt. It helps users choose movies.
...
For example, the system can use the following pre-set credentials:
 Username: Estrella Password: 12345 The user must enter this information correctly to continue.
...
Then at the bottom of the four that contain the information about the products The indicator shows the quantity of products obtained by the customer; 
then, to complete the purchase in this section, the "next" button must be selected; this will wait for the information which will be used in 
the purchase receipt window; on the other hand, if you wish to change something in the previous window, you must Select the "Back" 
...
The cost of each item. The total cost of an item multiplied by its quantity. The sum of all subtotals per item. Any tax applied to the purchase. 
Discounts/Promotions: Any price reduction applied. The final amount that the user has paid. Payment Information: 
It usually indicates how the payment was made, for example:

Payment method: Credit card, PayPal, bank transfer, etc.

Date and time of the transaction: When the purchase was completed.

Contact and Support Information: Includes data so that the user can contact the seller in case of doubts or problems:

* **Order number or transaction ID:** A unique identifier to reference the purchase.
...
3. Open the Visual Studio Code terminalIn the top bar, go to:
View → TerminalA terminal will open at the bottom.
4. Run the file in the terminalLocate the Run or play button, click on it,
 and the window generated by wxGlade will run correctly.
